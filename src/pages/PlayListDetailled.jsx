...
const PlayListDetailled = () => {
  ...
  const getPlaylistContent = async () => {
    try {
      ...
      console.log(
        thisPlalist.tracks.items.map((track) => track.track.artists[0].name)
      );
    } catch (error) {
      console.log(error);
    }
        ...
        <div className="page__content">
          <div className="playlist__cover--container">
            ...
            <div className="cover__details--container">
              <h3>PlayList</h3>
              <h3 className="cover__details__name">{thisPlalist.name}</h3>
            </div>
          </div>
          <div>
            <h3>Titres</h3>
          ...
          <div>
            {thisPlalist.tracks.items.length <= 0 ? (
              <LoadingData />
            ) : (
              thisPlalist.tracks.items.map(
                (playlistTrack) =>
                  playlistTrack && (
                    <PlayListTrackCard
